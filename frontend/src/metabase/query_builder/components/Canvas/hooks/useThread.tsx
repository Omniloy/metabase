...
export function useThread(userId: string) {
  ...
  const getOrCreateAssistant = async () => {
    ...
    const client = createClient();
    try {
      const assistants = await client.assistants.search({ metadata: null, limit: 50, offset: 0 });
      console.log("🚀 ~ getOrCreateAssistant ~ assistants:", assistants)
      // const assistant = await client.assistants.create({
      //   graphId: "get_data_agent",
      // });
      // console.log("🚀 ~ getOrCreateAssistant ~ assistant:", assistant)
    ...
    console.log("SELECTED AGENT:", selectedAgent)
      ...
      setCookie(ASSISTANT_ID_COOKIE, selectedAgent?.assistant_id);
    } catch (e) {
